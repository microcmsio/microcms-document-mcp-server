...
*   (\*6) Hobbyプランは対象外です。
*   (\*7) JSONフォーマットは[WRITE API](https://document.microcms.io/content-api/post-content)の指定方法と同様です。なお、CSVフォーマットの仕様上、テキストエディタなどから編集する際には`"`は`""`で登録する必要があります。
...
*   CSVインポートによるコンテンツ追加では[Webhook](/manual/webhook-setting)は通知されません。
