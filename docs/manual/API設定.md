...
コンテンツを新規作成する際の初期値を、フィールドごとに設定できます。詳細は各フィールドの詳細ページをご確認ください。

初期値を設定する際は、以下の点にご留意ください。

*   初期値を設定した後に新規作成されたコンテンツにのみ適用されます
*   コンテンツをコピーして新規作成した場合、初期値は反映されません
*   他のバリデーション設定によっては、設定した初期値を利用できない場合があります
