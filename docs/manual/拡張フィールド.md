...
`microcms-field-extension` は、新規利用を推奨し、継続的に機能改善・保守を行うメンテナンス方針（`Active`）としています。SDKの保守方針については、「制限事項／注意事項」ページの「[SDKの保守方針](https://document.microcms.io/manual/limitations#h8e929adf81)」をご確認ください。
