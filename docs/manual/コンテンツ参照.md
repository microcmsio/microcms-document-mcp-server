...
*   コンテンツ一覧画面では初期状態として参照先コンテンツの「コンテンツID」が表示されます。
*   コンテンツ一覧画面に表示する項目を変更したい場合は、「API設定 > APIスキーマ > 詳細設定 > 一覧画面に表示する項目」から変更できます。設定できる項目は[詳細設定](/manual/relation#h4ba8f153e0)をご確認ください。
