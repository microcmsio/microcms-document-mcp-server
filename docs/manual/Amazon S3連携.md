...
これにより、自社のポリシーに基づいたセキュリティ設定を適用できるようになるため、セキュリティ要件の高いプロジェクトでも安心してお使いいただけます。
...
*   メディアの削除
*   メディア名のリネーム（「メディアのファイル名を固定する」がOFFの場合のみ）
*   メディアの再アップロード
    *   ファイル名およびURLの挙動は下記の「[microCMS管理画面での設定](/manual/amazon-s3-integration#ha62f8dfa73)」をご参照ください
...
ONにした場合、ファイル名が変更できなくなります。  
  

**【再アップロード時の挙動】**

・ONのとき：同じファイル名・同じURLで上書きされます（異なるファイル名での再アップロードは不可）  
・OFFのとき：同じファイル名なら同じURL、異なるファイル名なら新しいURLになります
