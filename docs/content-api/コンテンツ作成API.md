...
下書き状態、または公開終了状態でコンテンツを作成するためのパラメータです。

*   下書き状態：`status=draft`をパラメータとして追加してください。
*   公開終了状態：`status=closed`をパラメータとして追加してください。
...
リクエストボディでフィールドに対して値を指定しない場合、APIスキーマで初期値が設定されているフィールドであっても、初期値は適用されません。
