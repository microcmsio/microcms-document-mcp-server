...
`q`パラメータによる検索は、コンテンツ内容をもとに作成された、全文検索用のデータを対象に実施されます。全文検索の仕様上、大文字・小文字、記号、スペース、文字列の区切り方などによって検索結果が変わる場合があります。特定のフィールドに対して、文字列の正確な部分一致検索が必要な場合は、`filters`パラメータの`contains`の使用をご検討ください。
...
*   テキストフィールド／テキストエリア／リッチエディタを横断して全文検索をしたい場合、q パラメータを利用してください。
