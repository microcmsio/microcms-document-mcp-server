...
下書きのコンテンツを更新・追加するためのパラメータです。`status=draft`をパラメータとして追加してください。  
更新時の挙動は「[リクエスト成功時の各ステータスごとの挙動](#h7d89d7da98)」をご参照ください。
...
公開中のステータスのまま、コンテンツが更新されます。

公開中かつ下書き中

公開中のステータスのコンテンツが更新されます。

下書き中のステータスのコンテンツは変わりません。

下書き中

下書き中のステータスのまま、コンテンツが更新されます。
...
`status=draft`を指定してリクエストした際の挙動は以下になります。
...
ステータスが「公開中かつ下書き中」になります。

公開中のコンテンツは維持されたまま、更新内容が下書き中のコンテンツとして保存されます。

公開中かつ下書き中

ステータスは「公開中かつ下書き中」のままです。

下書き中のコンテンツのみが更新され、公開中のコンテンツは維持されます。
